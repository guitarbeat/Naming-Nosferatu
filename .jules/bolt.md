...
## 2025-07-28 - Optimize performance observer iterations
**Learning:** Functional array methods like `.find()` instantiate callbacks each time, and standard `for...of` loops create iterator overhead. In performance-critical observers handling multiple metrics (like `layout-shift` or `paint`), processing can be optimized significantly using standard indexed `for` loops. Furthermore, operations like `parseFloat(clsTotal.toFixed(4))` should be executed outside the batch loop.
**Action:** Replace `entries.find()` and `for (const entry of entries)` with index-based `for (let i = 0; i < entries.length; i++)` in Web Vital observers, and defer expensive number formatting until the iteration batch concludes.
