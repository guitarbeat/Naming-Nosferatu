...
## 2024-07-02 - Chained array methods and garbage collection
**Learning:** Chained functional array methods like `.filter().map()` create intermediate arrays. In React render cycles with large payloads (like the leaderboard array), this causes significant garbage collection overhead and drops frames.
**Action:** Replace `.filter().map()` chains with single-pass `for` loops when dealing with large, frequently-rendered data arrays to avoid intermediate allocations.
