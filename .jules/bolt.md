...
## 2025-06-25 - Avoid array method chains in hot-path cache key generators
**Learning:** Chaining array methods like `.map().filter().map()` or `.reduce()` in frequently called functions (like `createSortedKey` and `getCacheKey`) allocates significant garbage and slows down iteration compared to a single-pass `for` loop that performs filtering and mapping simultaneously.
**Action:** Replace `map/filter` array method chains with single-pass `for` loops in hot-path functions, pushing results to a pre-allocated or newly created valid array before calling `.join()`. Retain `.sort()` where necessary for cache key correctness.
