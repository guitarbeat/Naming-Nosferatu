...
## 2025-06-25 - Avoid O(N) functional array mapping with chained slices
**Learning:** Chained array transformations like `.filter().slice(0, limit).map()` create multiple intermediate arrays resulting in poor performance on large datasets. They force complete O(N) enumeration even when only `limit` elements are required.
**Action:** Replace functional array method chains with a single `for` loop that short-circuits (`data.length >= limit`) to collapse iteration loops into a single O(limit) pass and eliminate temporary allocations.
