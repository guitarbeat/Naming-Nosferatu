...
## 2024-07-01 - Monolithic Component Smash
**Learning:** Monolithic UI components are harder to test and lack playfulness. By separating state logic (in `ProfileInner`) from view layers (`ProfileAvatar`, `ProfileView`, `ProfileEditForm`), we can safely introduce `framer-motion` animations to the views without muddying the main logic file.
**Action:** Always scan large `.tsx` files for multiple inline functional components and extract them early to improve maintainability.
