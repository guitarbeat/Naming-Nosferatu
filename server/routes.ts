...
router.post("/api/names/:id/toggle-hidden", async (req, res) => {
  try {
    ...
    await db
      ...
      .set({ isHidden })
      .where(eq(catNameOptions.id, req.params.id));
    res.json({ success: true });
  } catch (error) {
...
router.get("/api/analytics/popularity", async (req, res) => {
  try {
    ...
    if (targetUser) {
      ...
      ratingsQuery = ratingsQuery.where(
        eq(catNameRatings.userName, targetUser)
      ) as any;
    }
    ...
    const [selections, ratings, names] = await Promise.all([
      ...
      db
        ...
        .from(catNameOptions)
        .where(
          and(eq(catNameOptions.isActive, true), eq(catNameOptions.isHidden, false))
        ),
    ]);
    ...
    const analytics = names.map((name) => {
      ...
      const avgRating =
        rat.count > 0 ? Math.round(rat.totalRating / rat.count) : 1500;
...
router.post("/api/settings/cat-chosen-name", async (req, res) => {
  try {
    const { first_name, middle_names, last_name, greeting_text, show_banner } =
      req.body;
    const [inserted] = await db
