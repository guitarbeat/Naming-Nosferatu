...
function analyzeSounds() {
  ...
  const sounds = fs.readdirSync(SOUNDS_DIR).filter(file =>
  ...
  console.log(`Found ${sounds.length} sound files (${Math.round(totalSize / 1024 / 1024 * 100) / 100} MB total):\n`);
  ...
  soundInfo.forEach(sound => {
    ...
    try {
      ...
      if (optimizedSize < originalSize && optimizedSize > 0) {
        ...
        console.log(`     ${Math.round(originalSize/1024/1024*100)/100} MB → ${Math.round(optimizedSize/1024/1024*100)/100} MB`);
