...
function optimizeImages() {
  ...
  Object.entries(groups).forEach(([baseName, files]) => {
    ...
    console.log(`  Found ${files.length} formats: ${sortedFiles.map(f => `${f.ext}(${Math.round(f.size/1024)}KB)`).join(', ')}`);
...
function optimizeSounds() {
  ...
  try {
    ...
    console.log('  ⚠️  FFmpeg not available. Install it for audio optimization: https://ffmpeg.org/download.html');
  }
}
