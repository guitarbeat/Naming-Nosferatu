...
async function testConnection() {
    try {
        ...
        const { data, error } = await supabase.from('cat_name_options').select('count', { count: 'exact', head: true });

        if (error) {
            console.error("Connection Failed:", error);
            // Check for common error codes
            if (error.code === 'PGRST301') console.error("Hint: Client may be blocked or table does not exist.");
        } else {
