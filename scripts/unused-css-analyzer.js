...
class UnusedCSSAnalyzer {
    constructor() {
        this.cssSelectors = new Map(); // selector -> { file, line, usage }
    ...
    extractCSSSelectors(cssContent, filePath) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            // Skip CSS custom properties and @rules
...
async function main() {
    ...
    try {
        ...
        if (unusedSelectors.length > 0) {
            console.log('\n❓ Would you like to remove these unused selectors? (This will modify your CSS files)');
            console.log('   Run with --remove flag to automatically remove them');

            if (process.argv.includes('--remove')) {
                await analyzer.removeUnusedSelectors();
                analyzer.generateRemovalReport();
            }
        }
