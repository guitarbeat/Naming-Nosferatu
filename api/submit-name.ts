...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === "GET") {
    ...
    if (req.query.name) {
      ...
      if (checkName.length === 0) {
        return res
          .status(400)
          .json({ error: "Name parameter cannot be empty" });
      }
  ...
  body: JSON.stringify({ name: 'rococo', description: 'An ornate art style' })
        ...
        queryParams: {
          ...
          example:
            "POST /api/submit-name?name=rococo&description=An+ornate+art+style",
        },
      },
      validation: {
  ...
  const name =
    bodyData.name ||
    (typeof queryData.name === "string"
      ? queryData.name
      : String(queryData.name || ""));
  const description =
