...
export function RatingRadarChart({
	...
	const top = [];
	// ⚡ Bolt Optimization: Replaced O(N) chained `.filter().slice(0, limit)` with an O(K)
	// short-circuiting for-loop. By stopping iteration as soon as the limit is reached,
	// we avoid scanning the potentially large leaderboard array and reduce garbage collection.
	if (limit > 0) {
