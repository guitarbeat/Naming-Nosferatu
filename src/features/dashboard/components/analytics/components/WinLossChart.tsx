...
export function WinLossChart({ leaderboard, limit = 8 }: WinLossChartProps) {
	const data = [];
	// ⚡ Bolt Optimization: Replaced O(N) chained `.filter().slice(0, limit).map()` with an O(K)
	// short-circuiting for-loop. Instead of scanning the entire leaderboard and allocating
	// intermediate arrays, we only iterate until we find enough valid items to fill the chart.
	if (limit > 0) {
