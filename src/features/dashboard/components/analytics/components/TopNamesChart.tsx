...
export function TopNamesChart({ leaderboard, limit = 8 }: TopNamesChartProps) {
	// ⚡ Bolt Optimization: Replace `.reduce()` with a single-pass `for` loop
	// Avoids dynamic object allocations per iteration and intermediate array pushes,
	// reducing garbage collection and CPU overhead on large datasets.
	const data: Array<{
		name: string;
		rating: number;
		fullName: string;
