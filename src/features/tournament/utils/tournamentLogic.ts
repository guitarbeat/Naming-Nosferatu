...
// ⚡ Bolt Optimization: Replacing O(N log N) `.reduce().sort().join()` and `.map().join()` chains
// with a single-pass `for` loop using string concatenation. Omit `.sort()` to prevent
// cache collision bugs where different bracket seedings mapped to the same cache key.
