...
function getCacheKey(
	...
	// ⚡ Bolt Optimization: Replace chained reduce/sort/join with single-pass string concatenation.
	// The entrants are already passed in a stable order based on bracket seeding, so sort()
	// adds O(N log N) overhead and can cause bugs by masking seeding differences.
	let entrantsKey = "";
	for (let i = 0; i < bracketEntrants.length; i++) {
		const str = String(bracketEntrants[i]);
		if (str) {
			entrantsKey += (entrantsKey.length > 0 ? "," : "") + str;
		}
	}
