		...
		<AnimatePresence>
			{streakBurst && (
				<motion.div
					...
					className={`pointer-events-none absolute top-[20%] z-30 ${streakBurst.side === "left" ? "left-3 sm:left-6" : "right-3 text-right sm:right-6"}`}
				>
					<div
					...
					>
						...
						<div className="mt-2 flex gap-1.5">
							{Array.from({
								...
								// Flame count is entirely static based on streakBurst, and re-renders shouldn't cause them to swap position
								const flameId = `flame-${streakBurst.key}-${i}`;
								return (
									<span
										key={flameId}
