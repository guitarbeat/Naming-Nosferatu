...
let queryState = {
	isPending: true,
	data: undefined as { names: unknown[] } | undefined,
...
vi.mock("framer-motion", async (importOriginal) => {
	...
	return {
		...mod,
		motion: {
			div: ({ children, whileHover, whileTap, ...props }: unknown) => (
				<div {...props}>{children}</div>
			),
			button: ({ children, whileHover, whileTap, ...props }: unknown) => (
...
describe("NameSelector", () => {
	...
	it("should display names from query data", () => {
		queryState = {
			isPending: false,
			data: {
				names: [{ id: "100", name: "CustomCat", description: "A custom cat" }],
			},
			error: null,
		};
		renderComponent();
	...
	it("should trigger selection toggle on clicking a name", () => {
		...
		const nameCard = screen.getByRole("button", {
			name: /Select name: CustomCat/i,
		});
	...
	it("should trigger deselection when a selected name is clicked", () => {
		mockStore.tournament.selectedNames = [
			{ id: "100", name: "CustomCat", description: "A custom cat" },
		...
		const nameCard = screen.getByRole("button", {
			name: /Deselect name: CustomCat/i,
		});
		fireEvent.click(nameCard);
	...
	it("should confirm admin hide action", async () => {
		...
		await waitFor(() => {
			expect(mockToggleHidden).toHaveBeenCalledWith({
				nameId: "100",
