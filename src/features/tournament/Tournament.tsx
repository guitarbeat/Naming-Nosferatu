...
import type { Match, NameItem, TournamentProps, VoteData } from "@/shared/types";
...
function TournamentContent({ onComplete, names = [], onVote }: TournamentProps) {
	...
	const handleVoteAdapter = useCallback(
		(winnerId: string, _loserId: string) => {
			if (onVote && currentMatchRef.current) {
				const match = currentMatchRef.current;
				const left =
					typeof match.left === "object"
						? match.left
						: { id: match.left, name: String(match.left), description: "" };
				const right =
					typeof match.right === "object"
						? match.right
						: { id: match.right, name: String(match.right), description: "" };
				...
				const voteData: VoteData = {
					match: {
						left: {
							...
							outcome: winnerId === leftId ? "win" : "loss",
						},
						right: {
							name: right.name,
							id: right.id,
							description: right.description || "",
							outcome: winnerId === rightId ? "win" : "loss",
						},
					},
					result: winnerId === leftId ? 1 : 0,
					ratings: {}, // Ratings updated in hook
	...
	const {
		...
		audioManager,
	} = useTournament(visibleNames, userName, { onVote: handleVoteAdapter });
	...
	const _setCatPictures = useAppStore((state) => state.uiActions.setCatPictures);
	...
	// Derived states
	const leftName = matchData?.leftName;
	const rightName = matchData?.rightName;
	const leftDescription = matchData?.leftDescription;
	const rightDescription = matchData?.rightDescription;
	const leftPronunciation = matchData?.leftPronunciation;
	const rightPronunciation = matchData?.rightPronunciation;
	const leftImg =
	...
	const handleVoteForSide = useCallback(
		(side: "left" | "right") => {
			...
			triggerVoteFeedback(winnerName);
			handleVoteWithAnimation(side);
		},
	...
	return (
		<div className="relative w-full h-full min-h-[600px] flex flex-col items-center justify-center overflow-hidden bg-gradient-to-br from-slate-900 via-purple-950 to-slate-900">
			{/* Top Bar: Progress & Stats */}
			<div className="absolute top-0 left-0 right-0 z-40 p-4 sm:p-6 flex justify-between items-start pointer-events-none">
				<div className="pointer-events-auto flex items-center gap-3">
					<div className="bg-black/30 backdrop-blur-md rounded-xl px-4 py-2 border border-white/10 shadow-lg">
						...
						<div className="text-[10px] text-white/40 mt-0.5">
							Match {currentMatchNumber} / {totalMatches}
						</div>
					</div>
				</div>
				...
				<div className="flex flex-col items-end gap-2 pointer-events-auto">
					<div className="flex gap-2">
						<button
							...
							className="glass-panel p-2.5 rounded-full flex items-center justify-center border border-white/10 hover:bg-white/10 transition-colors"
						...
						<button
							type="button"
							onClick={handleQuit}
							className="glass-panel p-2.5 rounded-full flex items-center justify-center border border-white/10 hover:bg-red-500/20 transition-colors group"
							title="Exit Tournament"
						>
							<X className="size-4 text-white/70 group-hover:text-red-300 transition-colors" />
						</button>
					</div>
					{etaMinutes > 0 && (
						<div className="bg-black/20 backdrop-blur-sm rounded-lg px-2 py-1 flex items-center gap-1.5 border border-white/5">
							<Clock className="size-3 text-white/40" />
							<span className="text-[10px] text-white/50">~{etaMinutes}m left</span>
						</div>
					)}
				</div>
			</div>
			...
			<div className="absolute top-0 left-0 right-0 h-1 bg-white/5 z-50">
				<motion.div
					className="h-full bg-gradient-to-r from-primary via-fuchsia-500 to-amber-400 shadow-[0_0_10px_rgba(168,85,247,0.5)]"
					initial={{ width: 0 }}
			...
			<main className="flex-1 w-full flex flex-col items-center justify-center relative px-4 sm:px-6">
				{/* Announcements */}
					...
					>
						...
						<div className="flex flex-col items-center justify-center gap-2 py-1 w-14 sm:w-20">
							...
							<div className="flex flex-col gap-1.5 w-full">
								<button
									type="button"
									onClick={handleUndoWithAnimation}
