...
export function useImageGallery({ isLightboxOpen: _isLightboxOpen }: { isLightboxOpen: boolean }) {
	...
	useEffect(() => {
		...
		const fetchAll = async () => {
			...
			try {
				const [supa, manifest] = await Promise.all([trySupabase(), tryStaticManifest()]);
