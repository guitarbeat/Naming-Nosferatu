...
import { batchUpdateVisibility, softDeleteName, toggleNameHidden, toggleNameLocked } from "@/features/names/mutations";
...
import { BarChart3, Eye, EyeOff, Loader2, Lock, Trash2 } from "@/shared/lib/icons";
...
export function AdminDashboard() {
        ...
        const softDeleteMutation = useMutation({
                mutationFn: ({ nameId }: { nameId: string | number }) => softDeleteName({ nameId }),
                onSuccess: async () => {
                        await queryClient.invalidateQueries({ queryKey: namesQueryKeys.all });
                },
        });
        ...
        const batchVisibilityMutation = useMutation({
                mutationFn: ({ nameIds, isHidden }: { nameIds: string[]; isHidden: boolean }) =>
                        batchUpdateVisibility({ nameIds, isHidden }),
                onSuccess: async () => {
                        await queryClient.invalidateQueries({ queryKey: namesQueryKeys.all });
                },
        });
                ...
                async (action: BulkAction) => {
                        ...
                        try {
                                if (action === "hide" || action === "unhide") {
                                        await batchVisibilityMutation.mutateAsync({
                                                nameIds: ids,
        ...
        const handleSoftDelete = useCallback(
                async (nameId: string | number) => {
                        ...
                        try {
                                await softDeleteMutation.mutateAsync({ nameId });
                        } catch (error) {
                                                                        ...
                                                                        <div key={name.id} className="py-3 sm:py-4">
                                                                                <div className="flex items-start sm:items-center justify-between gap-2">
                                                                                        ...
                                                                                        <div className="flex items-center gap-1 shrink-0">
                                                                                                ...
                                                                                                <Button
                                                                                                        onClick={() => void handleSoftDelete(name.id)}
                                                                                                        variant="ghost"
                                                                                                        size="small"
