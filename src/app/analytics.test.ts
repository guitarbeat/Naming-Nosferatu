...
describe("shouldEnableAnalytics", () => {
	...
	it("disables analytics for replit preview environments", () => {
		expect(
			shouldEnableAnalytics({ hostname: "workspace.replit.dev", isProd: true }),
		).toBe(false);
		expect(
	...
	it("disables analytics for hostnames containing local", () => {
		expect(
			shouldEnableAnalytics({ hostname: "my-local-test.com", isProd: true }),
		).toBe(false);
		expect(
			shouldEnableAnalytics({ hostname: "local.example.com", isProd: true }),
		).toBe(false);
		expect(
			shouldEnableAnalytics({ hostname: "mylocalapp.net", isProd: true }),
		).toBe(false);
	});
	...
	it("enables analytics for valid production hosts", () => {
		...
		expect(
			shouldEnableAnalytics({ hostname: "nosferatu.app", isProd: true }),
		).toBe(true);
	});
	...
	it("throws an error when called with null or undefined (runtime check)", () => {
		expect(() =>
			shouldEnableAnalytics(
				null as unknown as { hostname: string; isProd: boolean },
			),
		).toThrow();
		expect(() =>
			shouldEnableAnalytics(
				undefined as unknown as { hostname: string; isProd: boolean },
			),
		).toThrow();
	});
