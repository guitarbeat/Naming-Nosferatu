		...
		<>
							...
							{wordEntries.map(({ word }, i) => (
								...
								>
									<span className="gradient-heading">
										{i < wordEntries.length - 1 ? `${word}\u00a0` : word}
									</span>
								</motion.span>
							))}
						</span>
								...
								>
									...
									<div className="flex items-center gap-2.5">
										...
										<span className="rounded-full bg-primary/15 px-2.5 py-0.5 font-mono text-xs font-semibold text-primary">
											★{" "}
											{Math.round(
												hoveredEntry.name.avgRating ?? hoveredEntry.name.avg_rating ?? 1500,
											)}
										</span>
									</div>
									...
									{(() => {
										...
										return total > 0 ? (
											<div className="flex items-center gap-1.5 text-xs text-muted-foreground/70">
												...
												{winRate !== null && (
													<span className="ml-1 text-muted-foreground/50">{winRate}% win rate</span>
												)}
											</div>
										) : (
											<span className="text-xs text-muted-foreground/40">no matches yet</span>
										);
									})()}
			...
			>
				<SectionHeading
					...
					subtitle="Have a great cat name idea? Share it!"
				/>
				<NameSuggestionInner />
			</Section>
			...
			>
				<SectionHeading
					icon={User}
					title="Your Profile"
					subtitle="Log in to track your ratings and preferences"
				/>
