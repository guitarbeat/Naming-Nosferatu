...
describe('apiClient', () => {
	...
	describe('error handling', () => {
		it('throws an error with message from error object if ok is false', async () => {
			fetchMock.mockResolvedValueOnce(createMockResponse(false, 400, { message: 'Bad Request Data' }));
		...
		it('throws an error with error from error object if ok is false', async () => {
			fetchMock.mockResolvedValueOnce(createMockResponse(false, 500, { error: 'Internal Server Error' }));
