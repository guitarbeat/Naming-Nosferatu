...
function ContenderCardPreview({ name, description, archetypeIcon = "🐾" }: CardPreviewProps) {
	const displayName = name.trim() || "Contender Name";
	const displayLore = description.trim() || "Description will appear here.";
	const hasContent = Boolean(name.trim() || description.trim());
	const previewAvatar = getAvatarForName(name);

	return (
		<div className="relative overflow-hidden rounded-2xl border border-white/30 dark:border-white/15 bg-card/85 dark:bg-black/40 p-4 sm:p-5 shadow-[0_8px_32px_0_rgba(31,38,135,0.08),inset_0_1px_1px_0_rgba(255,255,255,0.3)] backdrop-blur-xl transition-all">
				...
				<div className="flex items-center gap-3 min-w-0">
					...
					<div className="min-w-0">
						<div className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-primary">
							...
							<span>Contender Preview</span>
						</div>
						<div className="text-sm font-extrabold text-foreground truncate">
							{hasContent ? displayName : "Contender Preview"}
						</div>
					</div>
				</div>
			...
			<div className="space-y-2">
				<div className="text-base sm:text-lg font-black tracking-tight text-foreground line-clamp-1">
					{hasContent ? (
						...
						<span className="text-muted-foreground/50 italic">Enter contender name...</span>
					)}
				</div>
				...
				{INSPIRATION_ARCHETYPES.map((cat) => {
					...
					return (
						<button
							...
							className="inline-flex shrink-0 items-center gap-1.5 rounded-full border border-white/25 dark:border-white/10 bg-white/40 dark:bg-white/5 backdrop-blur-md hover:bg-primary/15 hover:border-primary/40 px-3 py-1.5 text-xs font-medium text-foreground shadow-xs transition-all active:scale-95 disabled:opacity-50"
			...
			<div className="space-y-4">
				...
				<div className="space-y-1.5">
					...
					<Textarea
						...
						placeholder="Describe this cat or explain the name choice."
						rows={3}
