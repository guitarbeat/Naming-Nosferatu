...
describe("api module", () => {
	...
	describe("namesQueryOptions and fetchNames", () => {
		...
		it("fetches names including hidden items when includeHidden is true", async () => {
			// Seed candidates with a hidden name item
			...
			// Verify includeHidden = false filters out the hidden name
			...
			// Verify includeHidden = true includes the hidden name
		...
		it("seamlessly merges new default candidates if stored candidates are missing items", async () => {
			// Seed storage with an incomplete subset of candidates
			...
			// Custom candidate should exist alongside merged default candidates
	...
	describe("ratingsAPI", () => {
		...
		it("handles errors during saveRatings and logs a warning", async () => {
			...
			// Create a proxy object that throws on property access during candidate iteration
