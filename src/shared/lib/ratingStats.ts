...
export function calculatePercentile(
	...
	// Bolt optimization: Single O(n) pass instead of array copy + O(n log n) sort + O(n) filter loops
...
export function getPercentileRank(rating: number, allRatings: number[]): number {
	...
	// Bolt optimization: Single O(n) pass to count elements below rating instead of O(n log n) full sort
	let belowCount = 0;
