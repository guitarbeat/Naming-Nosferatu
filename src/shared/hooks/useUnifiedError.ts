...
import { ErrorManager, FormattedError } from '../services/errorManager';
...
export const useUnifiedError = (options: UseUnifiedErrorOptions = {}) => {
  ...
  const handleError = useCallback((error: unknown, metadata: Record<string, unknown> = {}) => {
    ...
    if (showToast && typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('show-error-toast', {
        detail: { message: formattedError.userMessage, severity: formattedError.severity }
      }));
    }
  ...
  const clearValidationError = useCallback((field: string) => {
    setErrorState(prev => ({
      ...
      hasError: prev.error !== null || prev.validationErrors.filter(e => e.field !== field).length > 0,
    }));
  }, []);
  ...
  return {
    ...
    getFieldError: (field: string) => errorState.validationErrors.find(e => e.field === field)?.message,
