...
export function useLocalStorage<T>(
	...
	// Keep onError in a ref so the debounced writer always has access to the latest callback
	...
	useEffect(() => {
		if (options.debounceWait && options.debounceWait > 0) {
			debouncedSetItemRef.current = debounce((val: T) => {
				if (IS_BROWSER) {
					const success = writeStorageJson(key, val);
					if (!success) {
	...
	// Flush pending debounced write immediately on unmount to prevent data loss
	useEffect(() => {
		return () => {
