...
const ErrorBoundaryFallback: React.FC<ErrorBoundaryFallbackProps> = ({
	...
	const diagnosticInfo = {
		...
		additionalInfo:
			(standardizedError as { additionalInfo?: Record<string, unknown> })
				?.additionalInfo || {},
	...
	const handleCopyDiagnostics = async () => {
		...
		const stackFramesArray = Array.isArray(diagnosticInfo.stackFrames)
			? diagnosticInfo.stackFrames
			: [];
		const parsedFrames =
			stackFramesArray.length > 0
				? (
						stackFramesArray as Array<{
							file?: string;
							line?: number;
							column?: number;
							functionName?: string;
...
${
	parsedFrames.length > 0
		? (
				diagnosticInfo.stackFrames as Array<{
					file?: string;
					line?: number;
					column?: number;
					functionName?: string;
...
## Debug Hints
${
	Array.isArray(diagnosticInfo.debugHints) &&
