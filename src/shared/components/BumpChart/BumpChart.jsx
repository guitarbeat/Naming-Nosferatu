...
export function BumpChart({
  ...
  // * Generate animation key from data to trigger re-animation when data changes
  // * This avoids setState in effect by deriving the key directly from data
  const animationKey = useMemo(() => {
    if (!animated || !data || data.length === 0) return 0;
    // * Create a hash-like key from data to trigger re-animation on changes
    return JSON.stringify(data.map((d) => ({ id: d.id, rankings: d.rankings })))
      .length;
  }, [data, animated]);
