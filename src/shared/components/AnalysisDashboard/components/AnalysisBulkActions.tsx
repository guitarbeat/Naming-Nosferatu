...
import { devError, exportNamesToCSV } from "../../../utils/coreUtils";
import { AnalysisButton, AnalysisToolbar } from "./AnalysisComponents";
...
export function AnalysisBulkActions({
	...
	const handleExport = useCallback(() => {
		...
		const fileName =
			(selectedNames?.length ?? 0) > 0 ? "selected-names" : "all-names";
		exportNamesToCSV(
			dataToExport as Array<{ name: string; [key: string]: unknown }>,
			fileName,
		);
