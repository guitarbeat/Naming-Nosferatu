...
async function getNamesFromSupabase(
	...
	let query = client.from("cat_name_options").select(selectColumns);
...
const mapSnakeToCamel = <T extends Record<string, unknown>>(
	obj: T,
): Record<string, unknown> => {
	return mapFields(obj, snakeToCamelCase);
};

const _mapCamelToSnake = <T extends Record<string, unknown>>(
	obj: T,
): Record<string, unknown> => {
...
const cleanupLocalStorage = (priorityKeys: string[] = []): void => {
	...
	console.log(
		`localStorage cleanup: removed ${Math.round(removedSize / 1024)}KB`,
	);
};
...
export const ratingsAPI = {
	saveRatings: async (
		...
		if (!supabaseClient || !userId) {
			...
			const fallbackData: Record<string, unknown> = JSON.parse(existingRaw);
