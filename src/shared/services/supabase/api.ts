...
export const statsAPI = {
	getSiteStats: async () => {
		try {
			return await api.get<unknown>("/analytics/site-stats");
		} catch {
			return {};
		}
	},
	...
	saveRatings: ErrorManager.createResilientFunction(
		async (
			...
			try {
				...
				if (!response?.success) {
					throw new Error(
						`Failed to save ratings: ${response?.error || "Unknown error"}`,
					);
				}
				...
				if (error instanceof Error && error.message.includes("fetch")) {
					try {
						...
						console.warn(
							"Ratings saved to localStorage fallback due to API unavailability",
						);
						return { success: true, count: Object.keys(ratings).length };
					} catch (fallbackError) {
						console.error(
							"Failed to save ratings to localStorage fallback:",
							fallbackError,
						);
					}
				}
