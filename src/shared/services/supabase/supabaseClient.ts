...
// initialization logic moved from client.ts
...
const SUPABASE_URL = getEnvVar("SUPABASE_URL") || "https://ocghxwwwuubgmwsxgyoy.supabase.co";
const SUPABASE_ANON_KEY = getEnvVar("SUPABASE_ANON_KEY") || "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9jZ2h4d3d3dXViZ213c3hneW95Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTAwOTgzMjksImV4cCI6MjA2NTY3NDMyOX0.93cpwT3YCC5GTwhlw4YAzSBgtxbp6fGkjcfqzdKX4E0";

let supabase: SupabaseClient<Database> | null = typeof window !== "undefined" ? (window.__supabaseClient ?? null) : null;
...
export const catNamesAPI = {
   ...
   * @param topN - Number of top names to track
   * @param periods - Number of time periods to show
   * @param options - Optional filters (e.g., dateFilter)
   * @returns { data: Array, timeLabels: Array }
   */
  async getRankingHistory(
    topN = 10,
    periods = 7,
    options: { periods?: number; dateFilter?: string } = {}
  ): Promise<{ data: Array<{ id: string; name: string; rankings: (number | null)[]; avgRating: number; totalSelections: number }>; timeLabels: string[] }> {
    try {
      if (!(await isSupabaseAvailable())) {
      ...
      (ratings || []).forEach((r: RatingRow) => {
        const existing = ratingMap.get(r.name_id);
        if (!existing || (r.rating && r.rating > existing.rating)) {
          ratingMap.set(r.name_id, { rating: r.rating || 1500, wins: r.wins || 0 });
        }
      });
      ...
      const rankingData = sortedNames.map((nameInfo) => {
        const rankings = dateKeys.map((dateKey) => {
          ...
          const dayEntries = Array.from(dayData.entries()).sort(
            (a, b) => (b[1] as DayEntry).count - (a[1] as DayEntry).count,
          );
