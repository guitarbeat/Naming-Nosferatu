...
describe("supabaseAuthAdapter", () => {
	...
	it("returns false if name or password is not provided to login", async () => {
		const client = {
			auth: {
		...
		await expect(supabaseAuthAdapter.login({ name: "Ada" })).resolves.toBe(
			false,
		);
		await expect(
			supabaseAuthAdapter.login({ password: "password123" }),
		).resolves.toBe(false);
