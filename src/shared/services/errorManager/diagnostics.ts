 ...
 * @param {Record<string, unknown>} metadata - Error metadata
 * @returns {Diagnostics} Diagnostics object
 */
 ...
 * @param {string} context - Error context
 * @param {Record<string, any>} metadata - Error metadata
 * @param {any} environment - Environment snapshot
  ...
  if (errorInfo.cause) {
    ...
    if (typeof errorInfo.cause === "string") {
      ...
      try {
        ...
        causeDetail = `Cause available but could not be stringified (${errorMessage})`;
      }
    }
...
function generateFingerprint(
  errorInfo: { type: string; name: string; message: string },
  context: string,
  metadata: Record<string, unknown>,
  environment: Record<string, unknown>,
) {
 ...
 * @param {Record<string, any>} metadata - Error metadata
 * @returns {string[]} Array of related identifiers
 */
function collectRelatedIdentifiers(metadata: Record<string, unknown>): string[] {
