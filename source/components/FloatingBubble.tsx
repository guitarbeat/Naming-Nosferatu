        ...
        >
            <div className="relative w-full h-full">
                ...
                {isHighlighted && !isPoppingOut && (
                    <div className="absolute inset-0 rounded-full ring-4 ring-green-400/60 animate-pulse" style={{ margin: '-4px' }} />
                )}
                ...
                {/* Glow effect */}
                <div
                    className={`absolute inset-0 rounded-full blur-sm ${isHighlighted
                ...
                {/* Main bubble */}
                <div className={`relative w-full h-full rounded-full overflow-hidden bg-white/90 backdrop-blur-sm border-2 shadow-lg ${isHighlighted ? 'border-green-400/70' : 'border-white/50'
                    }`}>
                    {profile.avatar_url && !imageError ? (
                        <>
                            {/* Placeholder shown while loading */}
                            <div
                                className={`absolute inset-0 flex items-center justify-center font-bold text-lg transition-opacity duration-300 ${isHighlighted
                        ...
                        <div className={`w-full h-full flex items-center justify-center font-bold text-lg ${isHighlighted
                                ? 'bg-gradient-to-br from-green-100 via-green-50 to-green-100 text-green-600'
                                : 'bg-gradient-to-br from-coral/20 via-rose-pink/20 to-lavender/20 text-coral'
                            }`}>
                            {profile.display_name?.[0]?.toUpperCase() || profile.username[0]?.toUpperCase()}
                    ...
                    >
                        <span className={`text-xs font-semibold px-2 py-1 rounded-full shadow-sm animate-fade-in ${isHighlighted
