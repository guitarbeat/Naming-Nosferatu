		...
		<div className={`charts-bump-chart-container ${className}`}>
			...
			>
				...
				{processedData.map((series) => {
					...
					return (
						<g key={series.id} role="graphics-object" aria-label={`Ranking line for ${series.name}`}>
							{/* Line */}
							<path
								d={path}
							...
							{series.rankings.map((rank, pointIndex) => {
								...
								const isPointHovered = hoveredPoint?.seriesId === series.id && hoveredPoint?.index === pointIndex;
									...
									<g key={`${series.id}-${pointIndex}`}>
										<circle
											...
											onMouseEnter={() => {
												...
												setHoveredPoint({ seriesId: series.id, index: pointIndex });
											}}
											onMouseLeave={() => {
											...
											style={{ transition: "r 200ms, opacity 200ms", cursor: "pointer" }}
										/>
										{isPointHovered && (
