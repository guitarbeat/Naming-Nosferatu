	...
	getNamesWithDescriptions: async (includeHidden: boolean = false) => {
		return withSupabase(async (client) => {
			let query = client
				...
				.select("id, name, description, created_at, avg_rating, is_active, is_hidden, status, provenance")
				.eq("is_active", true)
...
export const hiddenNamesAPI = {
	hideName: async (userName: string, nameId: string | number) => {
		return withSupabase(
			...
			{ nameId: String(nameId), success: false, error: "Supabase not configured" } as {
				nameId: string | number;
				success: boolean;
				error?: string;
			},
	...
	hideNames: async (userName: string, nameIds: (string | number)[]) => {
		return withSupabase(
			...
			nameIds.map((id) => ({ nameId: id, success: false, error: "Supabase not configured" })),
		);
	},
