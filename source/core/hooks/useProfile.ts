  ...
  try {
    ...
    selections.forEach((s: TournamentSelection) => {
      ...
      if (s.name_id) {
        ...
        if (
          selectedDate &&
          s.selected_at &&
          (!nameLastSelected[nameId] ||
            selectedDate > new Date(nameLastSelected[nameId]))
        ) {
    ...
    Object.keys(nameSelectionCounts).forEach((nameId) => {
      nameSelectionFrequency[nameId] =
        uniqueTournaments > 0
          ? Math.round(
              ((nameSelectionCounts[nameId] || 0) / uniqueTournaments) * 100,
            ) / 100
          : 0;
    });
    ...
    const sortedDates = selections
      .map((s: TournamentSelection) =>
        new Date(s.selected_at || Date.now()).toDateString(),
      )
      .sort()
    ...
    for (let i = 0; i < sortedDates.length; i++) {
      if (i === 0) {
        ...
        const dayDiff = Math.floor(
          (new Date(currentDate as string).getTime() -
            new Date(prevDate as string).getTime()) /
            (1000 * 60 * 60 * 24),
        );
        if (dayDiff === 1) {
          tempStreak++;
  ...
  const handleToggleVisibility = useCallback(
    async (nameId: string) => {
      ...
      try {
        ...
        showSuccess(
          currentlyHidden ? `"Name" is now visible` : `"Name" is now hidden`,
        );
        setHiddenNames((prev) => {
