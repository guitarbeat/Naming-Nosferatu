...
import {
    tournamentsAPI, EloRating, PreferenceSorter, calculateBracketRound
} from "./TournamentLogic";
import { ErrorManager } from "../../shared/services/errorManager";
import {
    shuffleArray, clearTournamentCache, devError, devLog, devWarn, isNameHidden, ratingsToArray, ratingsToObject
} from "../../shared/utils";
import type {
    NameItem, Match, MatchRecord, PersistentState, TournamentUIState
...
export function useTournamentManager({
    ...
    return {
        currentView, setCurrentView, isEditingName, tempName, setTempName, isProcessing, votingError,
...
export function useTournamentVote({
    isProcessing, isTransitioning, isError, currentMatch, handleVote, onVote, audioManager, setIsProcessing, setIsTransitioning, setSelectedOption, setVotingError
...
export function useNameManagementCallbacks(context: any) {
    const setAllNames = useCallback((value: NameItem[] | ((prev: NameItem[]) => NameItem[])) => {
        if (context?.setAllNames) {
            context.setAllNames(value);
        }
    }, [context]);
