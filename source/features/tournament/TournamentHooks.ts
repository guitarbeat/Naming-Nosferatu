...
export function useTournamentState(
  ...
  existingRatings:
    | Record<
        string,
        number | { rating: number; wins?: number; losses?: number }
      >
    | null
    | undefined,
  onComplete: (
...
export function useTournamentManager({
  ...
  const [currentView, setCurrentView] = useState<"tournament" | "photos">(
    "tournament",
  );
...
export function useKeyboardControls(
  ...
  handleVoteWithAnimation:
    | ((option: "left" | "right" | "both" | "neither") => void)
    | undefined,
  options: {
    onSelectLeft?: () => void;
    onSelectRight?: () => void;
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (key === "ArrowLeft") {
        ...
        handleVoteWithAnimation?.(selectedOption as any);
      ...
      try {
        ...
        if (onVote && currentMatch) {
          await onVote({
            match: {
              left: {
                name: currentMatch.left?.name || currentMatch.left,
                outcome:
                  option === "left" || option === "both" ? "win" : "loss",
              },
              right: {
                name: currentMatch.right?.name || currentMatch.right,
                outcome:
                  option === "right" || option === "both" ? "win" : "loss",
              },
            },
