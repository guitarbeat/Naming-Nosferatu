...
export const UndoBanner = memo(
  ({ undoExpiresAt, undoStartTime, onUndo }: any) => {
  ...
  <AnimatePresence>
    {showMatchResult && lastMatchResult && (
      <motion.div
        ...
        exit={{ opacity: 0, y: 20 }}
        className="fixed right-8 bottom-8 z-[1000] p-6 text-white bg-purple-600 rounded-lg shadow-lg"
      >
        🏆 {lastMatchResult}
      </motion.div>
    )}
...
export const RoundTransition = memo(
  ({ showRoundTransition, nextRoundNumber }: any) => (
    <AnimatePresence>
...
export const FirstMatchTutorial = memo(
        ...
        >
          <Card className="max-w-md p-8 text-center bg-slate-900 border border-white/10 rounded-2xl">
            <h2 className="text-2xl font-bold mb-4 text-white">
              Welcome to the Judge's Seat!
            </h2>
            <p className="mb-6 text-slate-300">
              Select your favorite name by clicking or using arrow keys. You can
              also choose both or skip.
            </p>
