...
function TournamentContent({ onComplete, existingRatings = {}, names = [], onVote }: TournamentProps) {
	const { showSuccess, showError } = useToast();
	const visibleNames = getVisibleNames(names);

	const audioManager = useAudioManager();
	const {
		selectedOption, setSelectedOption, isTransitioning, setIsTransitioning,
	...
	if (!currentMatch) return <div className={styles.tournament}><Loading variant="spinner" /><p>Preparing tournament...</p></div>;

	return (
		<div className={styles.tournament} role="main">
			<TournamentHeader roundNumber={roundNumber} currentMatchNumber={currentMatchNumber} totalMatches={totalMatches} progress={progress} />
			<TournamentControls
			...
			/>
			<UndoBanner onUndo={handleUndo} undoExpiresAt={Date.now() + 5000} undoStartTime={Date.now()} />
