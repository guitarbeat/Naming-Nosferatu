 ...
 */
const NameDiscovery: React.FC<{ userName: string }> = ({
  userName: _userName,
}) => {
  const {
    data: popularNames,
          ...
          >
            <div className={styles.nameContent}>
              ...
              <div className={styles.lifecycle}>
                <span
                  className={`${styles.statusBadge} ${styles[name.status]}`}
                >
                  {name.status}
                </span>
                {name.provenance?.length > 0 && (
                  <div
                    ...
                    title={`History: ${name.provenance.map((p: any) => p.action).join(" -> ")}`}
                  >
...
const RandomGenerator: React.FC<{ userName: string }> = ({
  userName: _userName,
}) => {
                ...
                <div className={styles.actions}>
                  ...
                  >
                    <Heart
                      size={16}
                      className={
                        favorites.has(generatedName) ? styles.favorited : ""
                      }
                    />
                  </Button>
                  <Button
...
const CategoryExplorer: React.FC<{ userName: string }> = ({
  userName: _userName,
}) => {
  const [selectedCategory, setSelectedCategory] = useState<string | null>(null);
  ...
  const tabs = useMemo(
    () => [
      ...
      },
      {
        ...
        content: (
          <Suspense
            fallback={<Loading variant="spinner" text="Loading Stats..." />}
          >
            <AnalysisDashboard
              userName={userName}
      ...
      },
      {
        ...
        content: (
          <Suspense
            fallback={<Loading variant="spinner" text="Loading Photos..." />}
          >
            <GalleryView />
          </Suspense>
        ),
